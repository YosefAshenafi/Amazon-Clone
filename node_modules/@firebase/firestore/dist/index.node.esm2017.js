...
sharedClientState, currentUser, maxConcurrentLimboResolutions, isPrimary) {
    const syncEngine = new SyncEngineImpl(localStore, remoteStore, sharedClientState, currentUser, maxConcurrentLimboResolutions);
    if (isPrimary) {
...
async function handleCredentialChange(syncEngine, user) {
    const syncEngineImpl = debugCast(syncEngine);
...
function enqueueListen(asyncQueue, eventManger, query, options, observer) {
    ...
    return () => {
        wrappedObserver.mute();
        asyncQueue.enqueueAndForget(() => eventManagerUnlisten(eventManger, listener));
    };
}
